...
public class LineCoverageTestFitness extends TestFitnessFunction {
	...
	private void setupDependencies() {
		...
		if (cds.isEmpty() && !goalInstruction.isRootBranchDependent())
			throw new IllegalStateException(
					"expect control dependencies to be empty only for root dependent instructions: " + toString());
	...
	public double getFitness(TestChromosome individual, ExecutionResult result) {
		...
		// Deactivate coverage archive while measuring fitness, since branchcoverage
		// fitness
		// evaluating will attempt to claim coverage for it in the archive
		boolean archive = Properties.TEST_ARCHIVE;
		Properties.TEST_ARCHIVE = false;
		// org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:=========");
		// for(String key:result.getTrace().getCoverageData().keySet()) {
		// org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:"+key);
		// }
//		for (String key : result.getTrace().getCoveredMethods()) {
//			org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw cover method:" + key);
//		}
	 ...
	 * 
	 * @see
	 * org.evosuite.testcase.TestFitnessFunction#compareTo(org.evosuite.testcase.
	 * TestFitnessFunction)
	 */
	@Override
