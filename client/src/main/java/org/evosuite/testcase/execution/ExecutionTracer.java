...
public class ExecutionTracer {
	...
	/**
	 * We need to disable the execution tracer sometimes, e.g. when calling equals
	 * in the branch distance function
	 */
	...
	/**
	 * Obviously more than one thread is executing during the creation of concurrent
	 * TestCases. #TODO steenbuck we should test if Thread.currentThread() is in the
	 * set of currently executing threads
	 * 
	 * @return
	 */
...
//		org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:enteredMethod "+classname+methodname);
//		for (StackTraceElement ele : Thread.currentThread().getStackTrace()) {
//			org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:trace" + ele);
//		}
	 ...
	 * This is not enabled by default, only some coverage criteria (e.g., LCSAJ) use
	 * it.
	 * 
	 * @param opcode
	 *            a int.
	...
	public static void passedBranch(int val, int opcode, int branch, int bytecode_id) {
		...
		ExecutionTracer tracer = getExecutionTracer();
		// logger.info("passedBranch val="+val+", opcode="+opcode+", branch="+branch+",
		...
		// "+AbstractVisitor.OPCODES[opcode]+" and val "+val+" in branch "+branch);
	...
	public static void passedBranch(Object val1, Object val2, int opcode, int branch, int bytecode_id) {
		...
		// + AbstractVisitor.OPCODES[opcode]); // +", val1="+val1+", val2="+val2+" in
		// branch "+branch);
		double distance_true = 0;
		double distance_false = 0;
	 ...
	 * 
	 * Since it was not clear whether the field method call constitutes a definition
	 * or a use when the instrumentation was initially added this method will
