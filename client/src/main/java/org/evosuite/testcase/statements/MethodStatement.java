...
public class MethodStatement extends EntityWithParametersStatement {
	...
	public MethodStatement(TestCase tc, GenericMethod method, VariableReference callee,
		...
		super(tc, method.getReturnType(), parameters, method.getMethod().getAnnotations(),
	...
	/**
	 * This constructor allows you to use an already existing VariableReference as
	 * retvar. This should only be done, iff an old statement is replaced with this
	 * statement. And already existing objects should in the future reference this
	 * object.
	 * 
	 * @param tc
	 *            a {@link org.evosuite.testcase.TestCase} object.
	 ...
	 *            a {@link java.util.List} object.
	 */
	public MethodStatement(TestCase tc, GenericMethod method, VariableReference callee, VariableReference retvar,
			List<VariableReference> parameters) {
		super(tc, retvar, parameters, method.getMethod().getAnnotations(),
				method.getMethod().getParameterAnnotations());

		if (retvar.getStPosition() >= tc.size()) {
			// as an old statement should be replaced by this statement
			throw new IllegalArgumentException("Cannot replace in position " + retvar.getStPosition()
					+ " when the test case has only " + tc.size() + " elements");
		}
...
//		org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:" +this.toString()+"   "+ method.getName());
...
//					org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:exe start");
					...
					try {
						...
						for (int i = 0; i < parameters.size(); i++) {
							...
							if (inputs[i] != null && !TypeUtils.isAssignable(inputs[i].getClass(), parameterTypes[i])) {
								...
								throw new CodeUnderTestException(new UncompilableCodeException("Cannot assign "
										+ parameterVar.getVariableClass().getName() + " to " + parameterTypes[i]));
							}
						}
...
//					org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:before invoke");
					Object ret = method.getMethod().invoke(callee_object, inputs);
//					org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:after invoke");
					...
					if (method.getReturnType() instanceof Class<?>) {
						...
						if (!returnClass.isPrimitive() && ret != null
								&& !returnClass.isAssignableFrom(ret.getClass())) {
							throw new CodeUnderTestException(new ClassCastException("Cannot assign "
									+ method.getReturnType() + " to variable of type " + retval.getType()));
						}
					}
...
//					org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:exe end" );
				}
	...
	public Statement copy(TestCase newTestCase, int offset) {
		...
		if (retval instanceof ArrayReference && !(m.getReturnValue() instanceof ArrayReference)) {
			...
			ArrayReference newRetVal = new ArrayReference(newTestCase, retval.getGenericClass(),
					((ArrayReference) retval).getArrayLength());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.evosuite.testcase.StatementInterface#replace(org.evosuite.testcase.
	 * VariableReference, org.evosuite.testcase.VariableReference)
	 */
	/** {@inheritDoc} */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.evosuite.testcase.Statement#getUniqueVariableReferences()
	 */
	/** {@inheritDoc} */
	@Override
	...
	public boolean mutate(TestCase test, TestFactory factory) {
		...
		if (!isStatic() && Randomness.nextDouble() < pParam) {
			...
			objects = objects.stream()
					.filter(var -> !(test.getStatement(var.getStPosition()) instanceof FunctionalMockStatement))
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.evosuite.testcase.StatementInterface#changeClassLoader(java.lang.
	 * ClassLoader)
	 */
	/** {@inheritDoc} */
	@Override
