...
//		org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw test:transform " + className);
		...
		/*
		 * To use COMPUTE_FRAMES we need to remove JSR commands. Therefore, we have a
		 * JSRInlinerAdapter in NonTargetClassAdapter as well as CFGAdapter.
		 */
...
//			org.evosuite.utils.LoggingUtils.getEvoLogger().info("lzw param:" + Properties.INSTRUMENT_CONTEXT
//					+ Properties.INSTRUMENT_METHOD_CALLS + Properties.INSTRUMENT_LIBRARIES);
		...
		if (TestSuiteWriterUtils.needToUseAgent()) {
			...
			/*
			 * If the class is serializable, then doing any change (adding hashCode, static
			 * reset, etc) will change the serialVersionUID if it is not defined in the
			 * class. Hence, if it is not defined, we have to define it to avoid problems in
			 * serialising the class, as reading Master will not do instrumentation. The
			 * serialVersionUID HAS to be the same as the un-instrumented class
			 */
